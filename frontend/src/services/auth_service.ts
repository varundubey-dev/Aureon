...
export async function signupRequest(
  ...
  const response =
    await client.post<SignupRequestResponse>(
      "/auth/signup/request",
      {
        name,
        email,
...
export async function verifySignupOtp(
  ...
  const response =
    await client.post<SignupOtpResponse>(
      "/auth/signup/verify",
      {
        email,
        otp,
...
export async function checkUsernameAvailability(
  ...
  const response =
    await client.post<UsernameAvailabilityResponse>(
      "/auth/username/check",
      {
        username,
      },
...
export async function verifyPasswordResetOtp(
  ...
  const response =
    await client.post<PasswordResetOtpResponse>(
